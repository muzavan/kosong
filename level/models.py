...
def newLevel(image,answer,hint,contributor,a_contributor):
	try:
    	...
    	query = "INSERT INTO `level`(`image`,`answer`,`hint`,`contributor`,`a_contributor`,`viewed`) VALUES(\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',0)" % (str(image),str(answer),str(hint),str(contributor),str(a_contributor))
    	cur.execute(query)
    	return True
